...
import { humanizePokeApiName, translateMoveLearnMethod, translatePokemonType } from '@/features/localization'
  ...
  return (
    <Card className="gap-4 py-5">
      ...
      <CardContent className="px-5">
        ...
        {state.status === 'error' && <p className="mt-3 text-sm text-muted-foreground" role="alert">PokeAPI no está disponible. La guía editorial de Perla sigue funcionando.</p>}
...
function SpeciesResources({ pokemon }: { pokemon: PokemonSummaryItem }) {
  const moves = async (signal: AbortSignal): Promise<LearnsetView> => {
    const selected = selectMovesForGame(await getPokemonGameMoves(pokemon.id, { signal }), pearl)
  ...
  const evolution = async (signal: AbortSignal) => {
    ...
    return selectEvolutionForGame(await getEvolutionChain(species.evolution_chain.url, { signal }), pearl)
  }

  const encounters = async (signal: AbortSignal): Promise<EncounterArea[]> => (
    selectEncountersForGame(await getPokemonEncounters(pokemon.id, { signal }), pearl)
  )

  const defense = async (signal: AbortSignal) => {
    const loaded = await getPokemon(pokemon.name, { signal })
    return resolvePokemonDefenseForGame(loaded, bundledTypesByName, { id: pearl.slug, generation: pearl.generation })
  }

  return (
    <div className="mt-6 grid gap-3 lg:grid-cols-2">
      <ResourcePanel title="Evolución en Perla" description="Solo ramas registradas para Diamante y Perla." icon={Dna} load={evolution}>
        {(chain) => chain ? (
      ...
      <ResourcePanel title="Dónde aparece" description="Encuentros filtrados exclusivamente por la versión pearl." icon={MapPin} load={encounters}>
        {(areas) => areas.length > 0 ? <ul className="space-y-2">{areas.slice(0, 16).map((area) => <li key={area.locationArea.name} className="rounded-lg bg-secondary/60 p-3"><p className="font-medium">{formatArea(area.locationArea.name)}</p><p className="mt-1 text-xs text-muted-foreground">Nivel {Math.min(...area.versions.flatMap((version) => version.details.map((detail) => detail.minLevel)))}–{Math.max(...area.versions.flatMap((version) => version.details.map((detail) => detail.maxLevel)))} · Perla</p></li>)}</ul> : <p className="text-sm text-muted-foreground">No hay encuentros salvajes registrados para esta especie en Perla.</p>}
      </ResourcePanel>

      <ResourcePanel title="Qué aprende" description="Movimientos y métodos del grupo Diamante y Perla." icon={BookOpen} load={moves}>
      ...
      <ResourcePanel title="Defensa en Generación IV" description="Tipos y multiplicadores históricos, sin aplicar reglas modernas." icon={Shield} load={defense}>
...
export function PearlDataExplorer() {
  ...
  const load = async () => {
    ...
    setState({ status: 'loading' })
    try {
      const regional = await getRegionalPokedex(pearl, { signal: nextController.signal })
