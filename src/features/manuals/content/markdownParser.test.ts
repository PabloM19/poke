...
describe('parseManualPage', () => {
  ...
  it('publica los 21 artículos de 21–112 con contenido real', () => {
    expect(publishedManualArticles).toHaveLength(21)
    ...
    expect(publishedManualArticles.at(-1)?.printReference.pages.at(-1)).toBe(112)
    expect(manualSourcePages.find((source) => source.page === 86)?.markdown)
      .not.toContain('Páginas 87–128')
    expect(JSON.stringify(publishedManualArticles.at(-1)?.blocks)).toContain('R4 convencional')
  })
})
