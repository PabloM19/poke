...
export function SpoilerPreferenceControl({
  compact = false,
  ...
  return (
    <div className={compact ? 'block text-sm font-medium' : 'block rounded-[var(--radius-lg)] text-sm font-medium'}>
      <label className={labelVisuallyHidden ? 'sr-only' : undefined} htmlFor="spoiler-level-select">Nivel de spoilers</label>
      <SelectField id="spoiler-level-select" value={level} onChange={(event) => setLevel(event.target.value as SpoilerLevel)} className={labelVisuallyHidden ? 'font-normal' : 'mt-3 font-normal'}>
