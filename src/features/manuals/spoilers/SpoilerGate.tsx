...
export function SpoilerGate({ level, title, children }: { level: SpoilerLevel; title?: string; children: ReactNode }) {
  ...
  return (
    <section className="rounded-2xl border border-border bg-gradient-to-br from-primary/10 via-card to-secondary p-5 sm:p-8" aria-labelledby="spoiler-gate-title">
      <div className="mb-4 flex size-12 items-center justify-center rounded-xl bg-secondary"><ShieldAlert className="size-6 text-primary" aria-hidden /></div>
      <p className="text-sm font-semibold uppercase tracking-widest text-primary">Contenido oculto por tu preferencia</p>
      <h1 id="spoiler-gate-title" className="mt-2 text-2xl font-semibold">{title ?? 'Contenido de Manuales'}</h1>
